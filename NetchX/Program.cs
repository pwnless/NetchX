...
{
    public static readonly ISingleInstanceService SingleInstance = new SingleInstanceService($"Global\\{nameof(NetchX)}");
...
#pragma warning disable VSTHRD002
    ...
    public static void Main(string[] args)
    {
        ...
        Directory.SetCurrentDirectory(Global.NetchXDir);

        // append .\bin to PATH
        var binPath = Path.Combine(Global.NetchXDir, "bin");
...
#endif
        // clean up old files
        Updater.CleanOld(Global.NetchXDir);
...
#pragma warning restore VSTHRD002
    ...
    {
        Log.Information("NetchX Version: {Version}", $"{UpdateChecker.Owner}/{UpdateChecker.Repo}@{UpdateChecker.Version}");
        Log.Information("OS: {OSVersion}", Environment.OSVersion);
        Log.Information("SHA256: {Hash}", $"{await Utils.Utils.Sha256CheckSumAsync(Global.NetchXExecutable)}");
    ...
    public static void CreateLogger()
    {
        Log.Logger = new LoggerConfiguration()
            ...
            .MinimumLevel.Debug()
#endif
            .WriteTo.Async(c => c.File(Path.Combine(Global.NetchXDir, Constants.LogFile),
