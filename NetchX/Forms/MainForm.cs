...
public partial class MainForm : Form
{
    ...
    public MainForm()
    {
        ...
        if (Flags.NoSupport)
            _mainFormText.Add(Name, new[] { "{0} ({1})", "NetchX", "No Support" });
    ...
    private void RemoveNetchXFirewallRulesToolStripMenuItem_Click(object sender, EventArgs e)
    {
        Firewall.RemoveNetchXFwRules();
    }
    ...
    private async void NewVersionLabel_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            string downloadDirectory = Path.Combine(Global.NetchXDir, "data");
            ...
            var updater = new Updater(updateFileFullName, Global.NetchXDir);
            ...
            Process.Start(Global.NetchXExecutable);
            Environment.Exit(0);
        }
    ...
    private void CopyLinkPictureBox_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (ModifierKeys == Keys.Control)
                text = ShareLink.GetNetchXLink(server);
            else
    ...
    private void Minimize()
    {
        ...
        {
            // Show a notification.
            NotifyTip(i18N.Translate("NetchX is now minimized to the notification bar, double click this icon to restore."));
