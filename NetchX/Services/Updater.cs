...
public class Updater
{
    ...
    internal void ApplyUpdate()
    {
        ...
        // update archive file must have a top-level directory "NetchX"
        var updateDirectory = Path.Combine(extractPath, "NetchX");
        if (!Directory.Exists(updateDirectory))
            throw new MessageException(i18N.Translate("Update file top-level directory not exist"));

        // {_tempDirectory}/extract/NetchX/NetchX.exe
        var updateMainProgramFilePath = Path.Combine(updateDirectory, "NetchX.exe");
        if (!File.Exists(updateMainProgramFilePath))
            throw new MessageException(i18N.Translate($"Update file main program not exist"));

        MarkFilesOld();

        // move {tempDirectory}\extract\NetchX to install folder
