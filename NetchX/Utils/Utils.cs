...
public static class Utils
{
    ...
    public static void RegisterNetchXStartupItem()
    {
        const string TaskName = "NetchX Startup";
        const string LegacyTaskName = "Netch Startup";
        var folder = TaskService.Instance.GetFolder("\\");
        var taskNames = new[] { TaskName, LegacyTaskName };

        if (Global.Settings.RunAtStartup)
        {
            foreach (var taskName in taskNames.Where(taskName => folder.Tasks.Any(task => task.Name == taskName)))
                folder.DeleteTask(taskName, false);

            var td = TaskService.Instance.NewTask();

            td.RegistrationInfo.Author = "NetchX";
            td.RegistrationInfo.Description = "NetchX run at startup.";
            td.Principal.RunLevel = TaskRunLevel.Highest;

            td.Triggers.Add(new LogonTrigger());
            td.Actions.Add(new ExecAction(Global.NetchXExecutable));
            ...
            TaskService.Instance.RootFolder.RegisterTaskDefinition("NetchX Startup", td);
        }
        else
        {
            foreach (var taskName in taskNames.Where(taskName => folder.Tasks.Any(task => task.Name == taskName)))
